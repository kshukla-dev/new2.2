...
import GlobeOrbit, { type Country } from '@/components/home/GlobeOrbit'
import RotatingCountry from '@/components/home/RotatingCountry'
import dynamic from 'next/dynamic'

// Countries cycled in the rotating "Hire in ___" hero line.
const ROTATE_COUNTRIES = [
  'Spain', 'Germany', 'the Netherlands', 'France', 'Italy', 'Poland',
  'India', 'the UK', 'the UAE', 'China', 'Hong Kong', 'Belgium',
]

// 3D globe (globe.gl) relies on WebGL / window, so load it client-side only.
const GlobeClouds = dynamic(() => import('@/components/home/GlobeClouds'), {
  ssr: false,
  loading: () => <div className="globe-clouds-canvas" aria-hidden="true" />,
})

// Countries shown side-by-side across the globe's curved top. Array order =
// left→right placement (roughly west→east). `landmark` points at an image in
// /public/globe/landmarks — until one exists, the item shows flag + name.
const ORBIT_ORDER = [
  { name: 'United Kingdom', code: 'GB', landmark: 'big-ben.avif', tagline: 'EOR • Payroll • Compliance' },
  { name: 'Belgium', code: 'BE', landmark: 'brussels.png', tagline: 'Local hiring expertise' },
  { name: 'Netherlands', code: 'NL', landmark: 'windmill.avif', tagline: 'EOR • Payroll' },
  { name: 'France', code: 'FR', landmark: 'eiffel.jpg', tagline: 'EOR • Payroll • Compliance' },
  { name: 'Spain', code: 'ES', landmark: 'sagrada-familia.webp', tagline: 'Local hiring expertise' },
  { name: 'Germany', code: 'DE', landmark: 'brandenburg-gate.webp', tagline: 'EOR • Payroll • Compliance' },
  { name: 'Italy', code: 'IT', landmark: 'colosseum.png', tagline: 'EOR • Payroll' },
  { name: 'Czech Republic', code: 'CZ', landmark: 'prague.png', tagline: 'Local hiring expertise' },
  { name: 'Poland', code: 'PL', landmark: 'warsaw.jpg', tagline: 'EOR • Payroll' },
  { name: 'United Arab Emirates', code: 'AE', landmark: 'burj-khalifa.jpg', tagline: 'EOR • Compliance' },
  { name: 'India', code: 'IN', landmark: 'taj-mahal.png', tagline: 'Local hiring expertise' },
  { name: 'China', code: 'CN', landmark: 'shanghai.jpg', tagline: 'HR Boutique • Payroll' },
  { name: 'Hong Kong', code: 'HK', landmark: 'hong-kong.webp', tagline: 'EOR • Compliance' },
]

const heroCountries: Country[] = ORBIT_ORDER.map((c) => ({
  name: c.name,
  code: c.code,
  landmark: `/globe/landmarks/${c.landmark}`,
  tagline: c.tagline,
}))
...
const globalDestinations = [
  { id: 'uk', name: 'United Kingdom', continent: 'Europe', rating: '4.8', icon: 'https://flagcdn.com/w40/gb.png', image: 'https://images.unsplash.com/photo-1513635269975-59663e0ac1ad?auto=format&fit=crop&q=80&w=400' },
  { id: 'in', name: 'India', continent: 'Asia', rating: '4.7', icon: 'https://flagcdn.com/w40/in.png', image: 'https://images.unsplash.com/photo-1524492412937-b28074a5d7da?auto=format&fit=crop&q=80&w=400' },
  { id: 'de', name: 'Germany', continent: 'Europe', rating: '4.9', icon: 'https://flagcdn.com/w40/de.png', image: 'https://images.unsplash.com/photo-1467269204594-9661b134dd2b?auto=format&fit=crop&q=80&w=400' },
  { id: 'it', name: 'Italy', continent: 'Europe', rating: '4.8', icon: 'https://flagcdn.com/w40/it.png', image: 'https://images.unsplash.com/photo-1520175480921-4edfa2983e0f?auto=format&fit=crop&q=80&w=400' }
]
...
export default function HomePage() {
  ...
  const [geHovered, setGeHovered] = useState(false)
  const isManualScrolling = useRef(false)

  const scrollGeCarousel = (dir: 'left' | 'right') => {
    const node = geCarouselRef.current;
    if (node) {
      isManualScrolling.current = true;
      const amount = 300;

      if (dir === 'left' && node.scrollLeft <= 0) {
        node.scrollLeft = node.scrollWidth / 2;
      } else if (dir === 'right' && node.scrollLeft >= node.scrollWidth / 2) {
        node.scrollLeft = 0;
      }
  ...
  useEffect(() => {
    ...
    const render = (time: number) => {
      const node = geCarouselRef.current;
      if (node && !isManualScrolling.current) {
        const delta = time - lastTime;
        node.scrollLeft += delta * 0.05;
...
.home-hero h1 {
  ...
  /* Design system: Display, capped at 64px. One fluid clamp = smooth from 320px→desktop, no breakpoint jump. */
...
.ge-world-img {
  ...
  object-position: center;
  display: block;
  filter: brightness(0) saturate(100%) invert(10%) sepia(91%) saturate(1637%) hue-rotate(206deg) brightness(93%) contrast(101%);
  opacity: 0.9;
}
...
.card-nl { top: 20%; left: 45%; }
.card-de { top: 38%; left: 47%; }
.card-pl { top: 25%; left: 54%; }
.card-in { top: 58%; left: 63%; }
.card-cn { top: 44%; left: 74%; }
...
.card-nl { top: 20%; left: 42%; }
.card-de { top: 35%; left: 45%; }
.card-pl { top: 22%; left: 52%; }
.card-in { top: 58%; left: 60%; }
.card-cn { top: 44%; left: 70%; }
...
.home-content-scale {
  zoom: 1;
        ...
        /* Full-globe hero: empty space up top (for content), the whole rounded
           globe below it, and only the BOTTOM of the globe sliced by overflow.
           The globe renders as a wide square so it spans the full width; the
           square is taller than the visible area, so its lower part is clipped.
           Tuning knobs (edit these three):
           --globe-view-h : total height of the hero (top space + visible globe)
           --globe-top    : empty space above the globe's rounded top
           --globe-size   : globe render size (square). Bigger = larger globe */
        .globe-section {
          /* Section height = where the globe gets cut (overflow:hidden).
             Lower this to slice the globe higher; keep it > --globe-top. */
          --globe-view-h: 880px;
          /* fixed so the globe always starts BELOW the hero content (raise this
             if the flags ever overlap the stats; lower it to pull the globe up) */
          --globe-top: 560px;
          --globe-size: 165vw;
          position: relative;
          width: 100%;
          height: var(--globe-view-h);
          overflow: hidden;
          /* Light-blue backdrop matching the navbar, fading to white at the bottom
             so it blends into the content below. */
          background: linear-gradient(180deg, #ebf3fc 0%, #ebf3fc 62%, #ffffff 100%);
        }
        /* Hero copy + email capture, centred in the empty space above the globe. */
        .globe-hero {
          ...
          max-width: 960px;
          margin: 0 auto;
          padding: clamp(28px, 6vh, 80px) 20px 0;
          text-align: center;
        }
        .gh-eyebrow {
          display: inline-block;
        ...
        .gh-title {
          font-family: "IBM Plex Serif";
          /* Scales with viewport so it stays on one line on desktop. */
        ...
        .gh-title span { color: #F4661B; }
        .gh-sub {
          font-size: clamp(16px, 1.4vw, 19px);
          line-height: 1.6;
          color: rgba(20, 51, 105, 0.92);
          max-width: 560px;
          margin: 0 auto 18px;
        }
        /* Secondary line (below the rotating headline), in the sans font. */
        .gh-lead {
          font-family: var(--sans);
        ...
        .gh-lead span { color: #F4661B; }
        .rotating-country-wrap {
          display: inline-flex;
          justify-content: flex-start;
          min-width: 6.5ch;
        }
        .rotating-country {
          ...
          color: #F4661B;
          white-space: nowrap;
          animation: rc-in 0.5s cubic-bezier(0.16, 1, 0.3, 1);
        }
        @keyframes rc-in {
        ...
        .gh-form {
          ...
          max-width: 460px;
          margin: 0 auto;
          box-shadow: 0 8px 22px rgba(20, 51, 105, 0.09);
          transition: border-color 0.2s, box-shadow 0.2s;
        }
        .gh-form:focus-within {
          border-color: rgba(239, 90, 15, 0.55);
          box-shadow: 0 8px 24px rgba(239, 90, 15, 0.15);
        }
        .gh-form input {
          flex: 1;
          min-width: 0;
        ...
        .gh-form input::placeholder { color: rgba(20, 51, 105, 0.6); }
        .gh-cta {
          flex: 0 0 auto;
          background: linear-gradient(180deg, #F97B2E 0%, #EF5A0F 100%);
          color: #fff;
          border: none;
          border-radius: 9999px;   /* fully rounded pill */
          padding: 9px 22px;
          ...
          text-decoration: none;
          cursor: pointer;
          box-shadow: 0 5px 14px rgba(239, 90, 15, 0.3);
          transition: background 0.2s, transform 0.2s, box-shadow 0.2s;
        }
        .gh-cta:hover {
          background: linear-gradient(180deg, #F0691A 0%, #DB4E0B 100%);
          transform: translateY(-1px);
          box-shadow: 0 8px 22px rgba(239, 90, 15, 0.42);
        }
        .gh-note {
          font-size: 13px;
          color: rgba(20, 51, 105, 0.75);
          margin: 12px 0 0;
        }
        /* Four key stats shown in the band below the hero form. */
        .gh-stats {
          display: flex;
          justify-content: center;
          align-items: flex-start;
        ...
        .gh-stat {
          flex: 0 0 auto;
          width: 172px; /* equal-width cards so all four carry the same weight */
          text-align: center;
        }
        .gh-stat-top {
        ...
        .gh-stat-top .ac-lg { color: #F4661B; }
        .gh-stat-sub {
          font-size: 14px;
          font-weight: 600;
          color: rgba(20, 51, 105, 0.85);
          margin-top: 6px;
          line-height: 1.35;
          white-space: normal;
        }
        @media (max-width: 640px) {
          .gh-stats { gap: 22px 24px; }
          .gh-stat { width: 138px; }
          .gh-stat-top { font-size: 22px; }
          .gh-stat-sub { font-size: 13px; }
        }
        @media (max-width: 640px) {
          .gh-title {
            white-space: normal;
          ...
          /* Compact single-row field on mobile — input + button on one line,
             so it takes far less vertical space than the stacked version. */
          .gh-form {
            flex-direction: row;
            align-items: center;
        ...
        .globe-section .globe-clouds-canvas {
          position: absolute;
          left: 50%;
          top: var(--globe-top);
          transform: translateX(-50%);
          ...
          max-width: none;
          margin: 0;
          /* Soften the vivid satellite imagery so it sits calmer with the clean page. */
          filter: saturate(0.82) brightness(1.04);
        }
        /* Countries orbiting the globe along an ellipse. Positions/scale are
           computed per-frame in GlobeOrbit.tsx from the measured globe geometry. */
        .globe-orbit {
          position: absolute;
          inset: 0;
          overflow: hidden;
        ...
        .globe-orbit-line ellipse {
          fill: none;
          stroke: rgba(35, 76, 130, 0.24);
          stroke-width: 1.75;
        ...
        /* Flag + name badge, centred on its orbit point. */
        .orbit-content {
          position: absolute;
          left: 0;
          top: 0;
        ...
        .orbit-content:hover {
          filter: drop-shadow(0 6px 14px rgba(20, 51, 105, 0.25));
        }
        .orbit-flag {
          width: 58px;
          height: 39px; /* 3:2 flag ratio so the flag isn't cropped */
          border-radius: 9px;
          overflow: hidden;
          box-shadow: 0 3px 12px rgba(20, 51, 105, 0.22);
          background: #fff;
        ...
        .orbit-tooltip {
          ...
          box-shadow: 0 8px 24px rgba(20, 51, 105, 0.28);
          z-index: 5;
        }
        .orbit-tooltip strong { display: block; font-size: 13px; font-weight: 700; }
        .orbit-tooltip span { display: block; font-size: 11px; opacity: 0.85; margin-top: 2px; }
        .orbit-content:hover .orbit-tooltip {
          opacity: 1;
          transform: translateX(-50%) translateY(0);
        }
        /* ============================================================
           RESPONSIVE: GLOBE + FLAGS  (4 tiers)
           Knobs you can tune per breakpoint:
             --globe-view-h : total hero height (empty space + globe band)
             --globe-top    : how far down the globe starts (also lifts flags)
             --globe-size   : globe render size — bigger vw = larger globe
             .orbit-flag    : flag card size (keep width:height at 3:2)
             .orbit-name    : country label font-size (display:none hides it)
           BIG SCREEN (>1440px) uses the BASE values set above; the blocks
           below only override for smaller screens.
           (Motion/geometry knobs — ORBIT_FACTOR, ORBIT_LIFT, ORBIT_SECONDS,
            REPEAT, SCALE_MIN/MAX — live in GlobeOrbit.tsx.)
           ============================================================ */
        @media (max-width: 1440px) {   /* LAPTOP */
          ...
          .orbit-flag { width: 50px; height: 33px; }
          .orbit-name { font-size: 13px; }
          .gh-title { font-size: clamp(26px, 4.4vw, 46px); }
          .gh-lead { font-size: clamp(20px, 2.4vw, 28px); }
        }
        @media (max-width: 1024px) {   /* TABLET */
          .globe-section {
            --globe-view-h: max(96vh, 940px);
            --globe-top: 480px;   /* below the hero content */
            --globe-size: 150vw;  /* smaller globe */
          }
          .orbit-flag { width: 42px; height: 28px; }
          .orbit-name { font-size: 12px; }
          .gh-title { font-size: clamp(24px, 5vw, 40px); }
          .gh-lead { font-size: clamp(18px, 3vw, 26px); }
        }
        @media (max-width: 640px) {    /* MOBILE */
          .globe-section {
            --globe-view-h: 700px;  /* cut the earth to a band on mobile */
            --globe-top: 490px;     /* just below the stats */
            --globe-size: 150vw;
          }
          .globe-hero { padding-top: 20px; }
          .orbit-flag { width: 34px; height: 23px; }
          .orbit-name { display: none; }
          .gh-lead { font-size: clamp(17px, 5.5vw, 22px); }
          .gh-stats { gap: 16px 22px; margin-top: 28px; }
        }
`}</style>
      ...
      <section className="globe-section">
        <div className="globe-hero">
          <span className="gh-eyebrow">Your global HR partner</span>
          <h1 className="gh-title">Hire in <RotatingCountry words={ROTATE_COUNTRIES} /></h1>
          <p className="gh-lead">
            We handle <span>EOR</span>, <span>payroll</span> &amp; <span>immigration</span>
          </p>
          <p className="gh-sub">
            You build the team. We make hiring, payroll and relocation abroad fully compliant.
          </p>
          <div className="gh-form">
            <input type="email" placeholder="Your work email" aria-label="Your work email" />
            <Link href="/contact" className="gh-cta">Contact us</Link>
          </div>

          <div className="gh-stats">
            <div className="gh-stat">
              <div className="gh-stat-top"><b>17+</b> <span className="ac">countries</span></div>
              <div className="gh-stat-sub">in our network</div>
            </div>
            <div className="gh-stat">
              <div className="gh-stat-top"><b>15+</b> <span className="ac">local offices</span></div>
              <div className="gh-stat-sub">in countries we operate</div>
            </div>
            <div className="gh-stat">
              <div className="gh-stat-top"><b>&euro;200K</b> <span className="ac">saved</span></div>
              <div className="gh-stat-sub">vs. a local entity</div>
            </div>
            <div className="gh-stat">
              <div className="gh-stat-top"><b>700+</b> <span className="ac">companies</span></div>
              <div className="gh-stat-sub">trust us</div>
            </div>
          </div>
        </div>
        <GlobeClouds />
        <GlobeOrbit countries={heroCountries} />
      </section>
        ...
        <section className="ge-section">
          <div className="container ge-inner">
              ...
              <SlideIn direction="right" delay={0.2}>
                <div className="ge-map-visual">
                  <div className="ge-map-svg-wrap">
                    <img src="/case-study/contact-bg-removebg-preview.jpeg" alt="Global presence map" className="ge-world-img" loading="lazy" />
                  ...
                  <div className="ge-map-stats">
                    <div className="ge-map-stat"><NumberCounter as="strong" to={160} suffix="+" /><span>Countries</span></div>
                    <div className="ge-map-stat-divider"></div>
                    <div className="ge-map-stat"><NumberCounter as="strong" to={700} suffix="+" /><span>Businesses</span></div>
                    <div className="ge-map-stat-divider"></div>
                    <div className="ge-map-stat"><NumberCounter as="strong" to={48} suffix="h" /><span>Avg. onboarding</span></div>
                    <div className="ge-map-stat-divider"></div>
                    <div className="ge-map-stat"><NumberCounter as="strong" to={99.5} suffix="%" /><span>Client retention</span></div>
                  </div>
                </div>
              </SlideIn>
            </div>
            <SlideIn direction="up" delay={0.3}>
              <div
                className="ge-carousel-container"
                onMouseEnter={() => setGeHovered(true)}
                onMouseLeave={() => setGeHovered(false)}
              >
                <button className="ge-carousel-btn prev" onClick={() => scrollGeCarousel('left')} aria-label="Scroll left">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6" /></svg>
                </button>
                <div className="ge-carousel-wrapper" ref={geCarouselRef}>
                  <div className="ge-carousel">
                    {[...globalDestinations, ...globalDestinations, ...globalDestinations, ...globalDestinations].map((dest, idx) => (
                      <div key={dest.id + idx} className="ge-card" style={{ backgroundImage: `url('${dest.image}')` }}>
                        <div className="ge-card-overlay"></div>

                        <div className="ge-card-continent">
                          {dest.continent}
                        </div>

                        <div className="ge-card-content">
                          <h4 className="ge-card-name">{dest.name}</h4>

                          <div className="ge-card-bottom-row">
                            <Link href={'/' + dest.name.toLowerCase().replace(/\s+/g, '-')} className="ge-card-link">
                              Learn more
                            </Link>

                            <div className="ge-card-flag-rect">
                              <img src={dest.icon} alt={dest.name} loading="lazy" />
                            </div>
                          </div>
                        </div>
                      </div>
                ...
                <button className="ge-carousel-btn next" onClick={() => scrollGeCarousel('right')} aria-label="Scroll right">
